...
/* Simple I2C read/write */


uint8_t I2cMcuWrite( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size )
{
    uint8_t status = FAIL;
    uint16_t memAddSize = 0;
    ...
    status = ( HAL_I2C_Master_Transmit( i2c, deviceAddr, buffer, size, 2000 ) == HAL_OK ) ? SUCCESS : FAIL;
    return status;
}

uint8_t I2cMcuRead( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size)
{
    uint8_t status = FAIL;
    uint16_t memAddSize = 0;
    ...
    status = ( HAL_I2C_Master_Receive( i2c, deviceAddr, buffer, size, 2000 ) == HAL_OK ) ? SUCCESS : FAIL;
    return status;
}
