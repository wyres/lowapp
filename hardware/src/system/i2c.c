...
/* Custom I2C functions using Master_Transmit functions from the HAL */

uint8_t I2cSimpleWriteBuffer( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size )
{
    if( I2cInitialized == true )
    {
        if( I2cMcuWrite( obj, deviceAddr, buffer, size ) == FAIL )
        {
            // if first attempt fails due to an IRQ, try a second time
            if( I2cMcuWrite( obj, deviceAddr, buffer, size ) == FAIL )
            {
...
uint8_t I2cSimpleReadBuffer( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size )
{
    if( I2cInitialized == true )
    {
        return( I2cMcuRead(obj, deviceAddr, buffer, size ) );
    }
    else
    {
        return FAIL;
