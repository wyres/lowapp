...
/*!
 * \brief Write several data to the I2C device using HAL Master_Transmit (with no memory address)
 *
 * \param [IN] obj              I2C object
 * \param [IN] deviceAddr       device address
 * \param [IN] buffer           data buffer to write
 * \param [IN] size             number of bytes to write
 */
uint8_t I2cSimpleWriteBuffer( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size );

/*!
 * \brief Read several data byte from the I2C device using HAL Master_Transmit (with no memory address)
 *
 * \param [IN] obj              I2C object
 * \param [IN] deviceAddr       device address
 * \param [OUT] buffer          data buffer used to store the data read
 * \param [IN] size             number of data byte to read
 */
uint8_t I2cSimpleReadBuffer( I2c_t *obj, uint8_t deviceAddr, uint8_t *buffer, uint16_t size );
